...
- Grid cells fill the available space in both directions: cell width = `(pageWidth − (cols + 1) × spacing) / cols`, cell height = `(pageHeight − (rows − 1) × spacing) / rows`; icons and labels remain centred within each cell
